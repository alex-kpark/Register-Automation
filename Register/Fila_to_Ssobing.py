...
wb = xlrd.open_workbook('fila_data.xlsx')
...
for rownum in range(1, sh.nrows):
    ...
    name = row_values[2] + ' ' + row_values[28] + row_values[1]
    ...
    sum_content = row_values[3]
    outside_words = row_values[6]

    ops_name = '사이즈 및 색상'
    sex = row_values[39]

    #size_Val 정의 (옵션의 종류 선언)
    size_list = ['80', '85', '90', '95', '100', '105', '110']
    result = []
    for color in row_values[28:32]:
        if color == 'Null':
            ...
            result.append(color + ' ' + size_list[4] + ', ')
            result.append(color + ' ' + size_list[5] + ', ')
            result.append(color + ' ' + size_list[6] + ', ')
            
    size_val =  "".join(result)

    #inventory(재고) 정의
    inven_list = []
    for existing_stock in row_values[32:38]:
        if existing_stock == '':
            pass
        else:
            tb_integ = int(round(existing_stock))
